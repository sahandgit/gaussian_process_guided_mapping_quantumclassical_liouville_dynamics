...
The objective is to determine whether the single product-GP/moving-cloud/
explicit-MIDPOINT construction tested here can make the excess mapping-QCLE
operator computationally accessible on a one-dimensional, two-state
avoided-crossing benchmark. The Meyer--Miller--Stock--Thoss representation
...
source evaluated intrinsically on the midpoint cloud.  The update is formally
consistent with a second-order midpoint discretization of a smooth,
fixed-representation semi-discrete system. The required smooth moving-support
refit and safe-profile assumptions have not been established here, and the
production MIDPOINT calculations do not demonstrate positive
...
\cite{Church2018JCP,Cook2023JCTC,HairerLubichWanner2006}. The combined update
is therefore formally consistent with a second-order midpoint discretization
only for a smooth fixed-representation semi-discrete system. The
moving-support GP refit and safe-profile map used here are not shown to
satisfy the regularity assumptions needed to extend that statement to the
full production algorithm. A discontinuous floor activation or an unresolved
...
Whenever a source contribution is summarized as a ratio of global RMS norms,
that scalar is only a global diagnostic. It is not a pointwise bound on the
source update, and no statement such as ``less than one percent per step'' is
inferred for every support point.
...
pass a finite-output and endpoint physical-admissibility gate. Contraction is
then evaluated within each paired seed from \(D_{12,s}\) and \(D_{23,s}\), with
per-seed ratios, a four-seed contraction count, and a descriptive paired
Student-\(t\) interval for \(D_{12,s}-D_{23,s}\). Raw cross-seed observable
spread is retained as a cloud-variability diagnostic, not an uncertainty
estimate for these paired differences and not an order gate. Thus a ratio of
two roundoff-level quantities is never promoted to an observed order.
...
is claimed.
For PBME, all 14 admissible non-trace method--momentum--observable summaries
contract in all four paired seeds; the four trace summaries are limited by
Eq.~\eqref{eq:declared-numerical-noise-floor}. This explicitly demonstrates
within-cloud temporal contraction of the deterministic MInt propagation. For
MIDPOINT, 14 non-trace summaries stop at the physical-admissibility gate and
the two trace summaries stop at the numerical floor. At
\(P_{\rm init}=20\), for example, the mean \(\rho_{22}^{\rm SN}\) endpoints
are \(-0.180,-0.312,0.182\); at \(P_{\rm init}=100\), negative signed central
moments also occur. The 128 per-seed rows and 32 paired summaries retain every
\(D_{12,s}\), \(D_{23,s}\), ratio, contraction flag, and descriptive paired
interval in Table~\ref{tab:timestep-run-by-run} and the corresponding CSVs.
No MIDPOINT temporal order is interpreted from physically inadmissible levels,
and no deterministic production-order claim is made above the total
cloud-to-cloud variability.
...
The reported update is formally consistent with a second-order midpoint
discretization of a smooth fixed-representation semi-discrete system; the
moving-support refit and safe-profile assumptions required for the full
production algorithm have not been established. Its total error also contains
finite-support, product-model, nodal-floor,
...
% =========================================================
In summary, the explicit midpoint rule is formally second order for a smooth
fixed-representation semi-discrete source; this is not evidence of convergence
of the moving-support algorithm to the continuum
...
Time-step refinement & Dynamical observables at three common physical-time grids & $\Delta t=0.5,0.25,0.125$; $P_{\rm init}=20,100$; four paired seeds & Within-seed successive differences, ratios, contraction count, and descriptive paired interval \\
...
\tiny
\setlength{\tabcolsep}{3.5pt}
\begin{longtable}{l r l r r r r r r r L{0.15\textwidth}}
\caption{Paired time-step sensitivity. Endpoint values and time-normalized successive differences are averages over the same seeds 11, 29, 47, and 73 after interpolation to common physical times without extrapolation. The decision hierarchy is numerical floor, finite output, endpoint physical admissibility (populations in $[0,1]$, unit norm, finite energy, and nonnegative signed central second moments), then within-seed contraction. The final numeric column gives the mean paired difference $D_{12}-D_{23}$ and its descriptive two-sided paired Student-$t$ 95\% interval ($df=3$). Raw cross-seed observable spread remains archived as a cloud-variability diagnostic and is not an order gate.}\label{tab:timestep-refinement-physics}\\
\toprule
Method & $P_{\rm init}$ & Observable & $\langle y_{0.5}\rangle$ & $\langle y_{0.25}\rangle$ & $\langle y_{0.125}\rangle$ & $\langle D_{12}\rangle$ & $\langle D_{23}\rangle$ & Contract & $\overline{D_{12}-D_{23}}$ [95\%] & Interpretation \\
\midrule
\endfirsthead
...
\bottomrule
\endlastfoot
PBME & 20 & $\rho_{11}^{\mathrm{SN}}$ & $0.964$ & $0.964$ & $0.964$ & $7.02\times10^{-7}$ & $1.75\times10^{-7}$ & 4/4 & $5.26\times10^{-7}\,[4.22\times10^{-7},6.31\times10^{-7}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 20 & $\rho_{22}^{\mathrm{SN}}$ & $0.0357$ & $0.0357$ & $0.0357$ & $7.02\times10^{-7}$ & $1.75\times10^{-7}$ & 4/4 & $5.26\times10^{-7}\,[4.22\times10^{-7},6.31\times10^{-7}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 20 & electronic trace & $1$ & $1$ & $1$ & $2.90\times10^{-13}$ & $5.91\times10^{-13}$ & 0/4 & $-3.01\times10^{-13}\,[-3.07\times10^{-13},-2.95\times10^{-13}]$ & one or more seeds are numerical-floor limited; temporal order not interpreted \\
PBME & 20 & energy & $0.1$ & $0.1$ & $0.1$ & $4.93\times10^{-9}$ & $1.23\times10^{-9}$ & 4/4 & $3.70\times10^{-9}\,[3.58\times10^{-9},3.81\times10^{-9}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 20 & $\langle R\rangle$ & $15.3$ & $15.3$ & $15.3$ & $1.48\times10^{-6}$ & $3.69\times10^{-7}$ & 4/4 & $1.11\times10^{-6}\,[9.12\times10^{-7},1.31\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 20 & $\langle P\rangle$ & $19.8$ & $19.8$ & $19.8$ & $3.50\times10^{-6}$ & $8.74\times10^{-7}$ & 4/4 & $2.62\times10^{-6}\,[2.12\times10^{-6},3.12\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 20 & $M^{\rm signed}_{2,R}$ & $2.31$ & $2.31$ & $2.31$ & $4.89\times10^{-7}$ & $1.22\times10^{-7}$ & 4/4 & $3.67\times10^{-7}\,[1.83\times10^{-7},5.51\times10^{-7}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 20 & $M^{\rm signed}_{2,P}$ & $1.66$ & $1.66$ & $1.66$ & $3.00\times10^{-6}$ & $7.49\times10^{-7}$ & 4/4 & $2.25\times10^{-6}\,[1.78\times10^{-6},2.72\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & $\rho_{11}^{\mathrm{SN}}$ & $0.137$ & $0.137$ & $0.137$ & $6.74\times10^{-6}$ & $1.69\times10^{-6}$ & 4/4 & $5.06\times10^{-6}\,[4.88\times10^{-6},5.23\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & $\rho_{22}^{\mathrm{SN}}$ & $0.863$ & $0.863$ & $0.863$ & $6.74\times10^{-6}$ & $1.69\times10^{-6}$ & 4/4 & $5.06\times10^{-6}\,[4.88\times10^{-6},5.23\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & electronic trace & $1$ & $1$ & $1$ & $5.64\times10^{-14}$ & $1.16\times10^{-13}$ & 0/4 & $-5.98\times10^{-14}\,[-6.31\times10^{-14},-5.65\times10^{-14}]$ & one or more seeds are numerical-floor limited; temporal order not interpreted \\
PBME & 100 & energy & $2.5$ & $2.5$ & $2.5$ & $5.98\times10^{-8}$ & $1.50\times10^{-8}$ & 4/4 & $4.49\times10^{-8}\,[4.15\times10^{-8},4.83\times10^{-8}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & $\langle R\rangle$ & $14.9$ & $14.9$ & $14.9$ & $3.94\times10^{-7}$ & $9.85\times10^{-8}$ & 4/4 & $2.95\times10^{-7}\,[2.74\times10^{-7},3.17\times10^{-7}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & $\langle P\rangle$ & $99.1$ & $99.1$ & $99.1$ & $6.82\times10^{-6}$ & $1.70\times10^{-6}$ & 4/4 & $5.11\times10^{-6}\,[4.94\times10^{-6},5.28\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & $M^{\rm signed}_{2,R}$ & $0.985$ & $0.985$ & $0.985$ & $1.81\times10^{-7}$ & $4.51\times10^{-8}$ & 4/4 & $1.35\times10^{-7}\,[9.69\times10^{-8},1.74\times10^{-7}]$ & paired differences contract in all four seeds; interval is descriptive \\
PBME & 100 & $M^{\rm signed}_{2,P}$ & $0.444$ & $0.444$ & $0.444$ & $7.48\times10^{-6}$ & $1.87\times10^{-6}$ & 4/4 & $5.61\times10^{-6}\,[5.44\times10^{-6},5.77\times10^{-6}]$ & paired differences contract in all four seeds; interval is descriptive \\
MIDPOINT & 20 & $\rho_{11}^{\mathrm{SN}}$ & $1.18$ & $1.31$ & $0.818$ & $0.525$ & $0.422$ & 3/4 & $0.102\,[-0.196,0.401]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 20 & $\rho_{22}^{\mathrm{SN}}$ & $-0.18$ & $-0.312$ & $0.182$ & $0.525$ & $0.422$ & 3/4 & $0.102\,[-0.196,0.401]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 20 & electronic trace & $1$ & $1$ & $1$ & $1.45\times10^{-12}$ & $1.68\times10^{-12}$ & 1/4 & $-2.31\times10^{-13}\,[-1.82\times10^{-12},1.36\times10^{-12}]$ & one or more seeds are numerical-floor limited; temporal order not interpreted \\
MIDPOINT & 20 & energy & $0.107$ & $0.126$ & $0.0937$ & $0.0317$ & $0.0232$ & 4/4 & $0.00853\,[-0.00443,0.0215]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 20 & $\langle R\rangle$ & $16.6$ & $14.3$ & $13.9$ & $2.38$ & $2.21$ & 3/4 & $0.169\,[-0.434,0.773]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 20 & $\langle P\rangle$ & $21.5$ & $24.2$ & $18.5$ & $4.91$ & $3.96$ & 3/4 & $0.957\,[-0.991,2.9]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 20 & $M^{\rm signed}_{2,R}$ & $-1.52\times10^{3}$ & $-5.07$ & $-13.1$ & $503$ & $8.38$ & 4/4 & $495\,[-1.04\times10^{3},2.03\times10^{3}]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 20 & $M^{\rm signed}_{2,P}$ & $-2.67\times10^{3}$ & $11.4$ & $-13.4$ & $1.94\times10^{3}$ & $82.5$ & 4/4 & $1.86\times10^{3}\,[-3.86\times10^{3},7.57\times10^{3}]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & $\rho_{11}^{\mathrm{SN}}$ & $-0.201$ & $-0.198$ & $-0.18$ & $5.74$ & $5.82$ & 3/4 & $-0.0741\,[-1.94,1.79]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & $\rho_{22}^{\mathrm{SN}}$ & $1.2$ & $1.2$ & $1.18$ & $5.74$ & $5.82$ & 3/4 & $-0.0741\,[-1.94,1.79]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & electronic trace & $1$ & $1$ & $1$ & $7.61\times10^{-13}$ & $1.44\times10^{-12}$ & 0/4 & $-6.76\times10^{-13}\,[-1.58\times10^{-12},2.30\times10^{-13}]$ & one or more seeds are numerical-floor limited; temporal order not interpreted \\
MIDPOINT & 100 & energy & $2.5$ & $2.53$ & $2.52$ & $0.704$ & $0.722$ & 2/4 & $-0.0177\,[-0.565,0.53]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & $\langle R\rangle$ & $12.8$ & $12.5$ & $12.6$ & $14.5$ & $12.4$ & 2/4 & $2.03\,[-4.65,8.7]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & $\langle P\rangle$ & $98.8$ & $99.4$ & $99.2$ & $18.2$ & $18$ & 3/4 & $0.258\,[-12.8,13.3]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & $M^{\rm signed}_{2,R}$ & $-1.02\times10^{44}$ & $-1.27\times10^{43}$ & $-6.88\times10^{45}$ & $2.95\times10^{43}$ & $2.01\times10^{45}$ & 3/4 & $-1.98\times10^{45}\,[-8.30\times10^{45},4.33\times10^{45}]$ & one or more levels physically inadmissible; temporal order not interpreted \\
MIDPOINT & 100 & $M^{\rm signed}_{2,P}$ & $-5.96\times10^{45}$ & $-9.50\times10^{44}$ & $-4.79\times10^{47}$ & $3.06\times10^{45}$ & $2.38\times10^{47}$ & 3/4 & $-2.35\times10^{47}\,[-9.83\times10^{47},5.13\times10^{47}]$ & one or more levels physically inadmissible; temporal order not interpreted \\
\end{longtable}
\endgroup
\end{landscape}
...
\bottomrule
\endlastfoot
PBME & 20 & $\rho_{11}^{\mathrm{SN}}$ & $0.963\pm 0.0109$ & $0.964\pm 0.00965$ & $0.953\pm 0.00151$ & resolvable change exceeds seed dispersion \\
PBME & 20 & $\rho_{22}^{\mathrm{SN}}$ & $0.0369\pm 0.0109$ & $0.0356\pm 0.00965$ & $0.0472\pm 0.00151$ & resolvable change exceeds seed dispersion \\
PBME & 20 & electronic trace & $1\pm 7.72\times10^{-15}$ & $1\pm 3.67\times10^{-15}$ & $1\pm 1.12\times10^{-15}$ & roundoff-limited; cloud-size effect not interpreted \\
PBME & 20 & energy & $0.0999\pm 1.94\times10^{-4}$ & $0.1\pm 9.32\times10^{-5}$ & $0.1\pm 9.57\times10^{-5}$ & resolvable change remains within seed dispersion \\
PBME & 20 & $\langle R\rangle$ & $15.3\pm 0.033$ & $15.3\pm 0.0236$ & $15.3\pm 0.0264$ & resolvable change exceeds seed dispersion \\
PBME & 20 & $\langle P\rangle$ & $19.8\pm 0.0466$ & $19.8\pm 0.0583$ & $19.7\pm 0.013$ & resolvable change exceeds seed dispersion \\
PBME & 20 & $M^{\rm signed}_{2,R}$ & $2.18\pm 0.124$ & $2.37\pm 0.133$ & $2.31\pm 0.0335$ & resolvable change remains within seed dispersion \\
PBME & 20 & $M^{\rm signed}_{2,P}$ & $1.46\pm 0.0285$ & $1.7\pm 0.097$ & $1.7\pm 0.0634$ & resolvable change exceeds seed dispersion \\
PBME & 100 & $\rho_{11}^{\mathrm{SN}}$ & $0.137\pm 0.00679$ & $0.135\pm 0.0178$ & $0.143\pm 0.006$ & resolvable change remains within seed dispersion \\
PBME & 100 & $\rho_{22}^{\mathrm{SN}}$ & $0.863\pm 0.00679$ & $0.865\pm 0.0178$ & $0.857\pm 0.006$ & resolvable change remains within seed dispersion \\
PBME & 100 & electronic trace & $1\pm 9.02\times10^{-16}$ & $1\pm 1.78\times10^{-15}$ & $1\pm 3.51\times10^{-16}$ & roundoff-limited; cloud-size effect not interpreted \\
PBME & 100 & energy & $2.5\pm 9.52\times10^{-4}$ & $2.5\pm 4.71\times10^{-4}$ & $2.5\pm 4.84\times10^{-4}$ & resolvable change remains within seed dispersion \\
PBME & 100 & $\langle R\rangle$ & $14.9\pm 0.0205$ & $14.9\pm 0.0249$ & $14.9\pm 0.0124$ & resolvable change remains within seed dispersion \\
PBME & 100 & $\langle P\rangle$ & $99.1\pm 0.0226$ & $99.1\pm 0.0247$ & $99.1\pm 0.00766$ & resolvable change exceeds seed dispersion \\
PBME & 100 & $M^{\rm signed}_{2,R}$ & $0.979\pm 0.0751$ & $0.986\pm 0.054$ & $1\pm 0.0299$ & resolvable change remains within seed dispersion \\
PBME & 100 & $M^{\rm signed}_{2,P}$ & $0.426\pm 0.0187$ & $0.447\pm 0.0117$ & $0.46\pm 0.00392$ & resolvable change exceeds seed dispersion \\
MIDPOINT & 20 & $\rho_{11}^{\mathrm{SN}}$ & $1.08\pm 0.0635$ & $0.916\pm 0.0672$ & $0.951\pm 0.0744$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 20 & $\rho_{22}^{\mathrm{SN}}$ & $-0.079\pm 0.0635$ & $0.0841\pm 0.0672$ & $0.0485\pm 0.0744$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 20 & electronic trace & $1\pm 6.46\times10^{-13}$ & $1\pm 6.09\times10^{-13}$ & $1\pm 7.07\times10^{-13}$ & roundoff-limited; cloud-size effect not interpreted \\
MIDPOINT & 20 & energy & $0.101\pm 0.0214$ & $0.0994\pm 0.0167$ & $0.0934\pm 0.0207$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 20 & $\langle R\rangle$ & $16.9\pm 0.989$ & $14.9\pm 2$ & $14.5\pm 2.24$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 20 & $\langle P\rangle$ & $20.5\pm 1.93$ & $19.5\pm 1.73$ & $19\pm 1.75$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 20 & $M^{\rm signed}_{2,R}$ & $-1.31\times10^{3}\pm 262$ & $-9.5\pm 52.5$ & $-1.94\times10^{8}\pm 3.31\times10^{8}$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 20 & $M^{\rm signed}_{2,P}$ & $-1.94\times10^{3}\pm 571$ & $-6.33\pm 85.3$ & $-3.87\times10^{8}\pm 6.63\times10^{8}$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & $\rho_{11}^{\mathrm{SN}}$ & $0.923\pm 1.01$ & $-0.46\pm 0.98$ & $-0.297\pm 0.842$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & $\rho_{22}^{\mathrm{SN}}$ & $0.0775\pm 1.01$ & $1.46\pm 0.98$ & $1.3\pm 0.842$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & electronic trace & $1\pm 1.39\times10^{-13}$ & $1\pm 7.76\times10^{-14}$ & $1\pm 3.65\times10^{-14}$ & roundoff-limited; cloud-size effect not interpreted \\
MIDPOINT & 100 & energy & $2.41\pm 0.0692$ & $2.53\pm 0.107$ & $2.5\pm 0.05$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & $\langle R\rangle$ & $12.4\pm 3.43$ & $12.5\pm 0.978$ & $12.6\pm 1.27$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & $\langle P\rangle$ & $98.2\pm 0.412$ & $99.2\pm 1.28$ & $98.7\pm 1.29$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & $M^{\rm signed}_{2,R}$ & $-1.47\times10^{19}\pm 2.54\times10^{19}$ & $-1.69\times10^{43}\pm 2.56\times10^{43}$ & $-4.76\times10^{94}\pm 8.25\times10^{94}$ & physically inadmissible output; cloud-size comparison not meaningful \\
MIDPOINT & 100 & $M^{\rm signed}_{2,P}$ & $-8.89\times10^{20}\pm 1.53\times10^{21}$ & $-1.27\times10^{45}\pm 1.96\times10^{45}$ & $-2.43\times10^{96}\pm 4.20\times10^{96}$ & physically inadmissible output; cloud-size comparison not meaningful \\
\end{longtable}
\endgroup
\end{landscape}
...
PBME & 20 & $M^{\rm signed}_{2,R}$ & $2.31$ & $0.163$ & $[2.05,2.57]$ & $0.357$ \\
PBME & 20 & $M^{\rm signed}_{2,P}$ & $1.66$ & $0.118$ & $[1.47,1.85]$ & $0.281$ \\
...
PBME & 100 & $M^{\rm signed}_{2,R}$ & $0.985$ & $0.0441$ & $[0.915,1.06]$ & $0.0944$ \\
PBME & 100 & $M^{\rm signed}_{2,P}$ & $0.444$ & $0.0116$ & $[0.426,0.463]$ & $0.021$ \\
...
MIDPOINT & 20 & $M^{\rm signed}_{2,R}$ & $-5.07$ & $43.8$ & $[-74.8,64.6]$ & $104$ \\
MIDPOINT & 20 & $M^{\rm signed}_{2,P}$ & $11.4$ & $78.1$ & $[-113,136]$ & $170$ \\
...
MIDPOINT & 100 & $M^{\rm signed}_{2,R}$ & $-1.27\times10^{43}$ & $2.25\times10^{43}$ & $[-4.85\times10^{43},2.31\times10^{43}]$ & $4.63\times10^{43}$ \\
MIDPOINT & 100 & $M^{\rm signed}_{2,P}$ & $-9.50\times10^{44}$ & $1.72\times10^{45}$ & $[-3.69\times10^{45},1.79\times10^{45}]$ & $3.52\times10^{45}$ \\
\end{longtable}
\endgroup
\end{landscape}
...
\begin{center}
\small
\begin{tabular}{L{0.43\textwidth}rrl}
\toprule
MInt implementation diagnostic & Residual & Acceptance tolerance & Status \\
\midrule
one--step symplectic residual & $4.89\times10^{-8}$ & $<1.00\times10^{-6}$ & PASS \\
one--step round--trip residual & $2.36\times10^{-16}$ & $<1.00\times10^{-10}$ & PASS \\
mapping--radius drift & $1.47\times10^{-14}$ & $<1.00\times10^{-11}$ & PASS \\
trajectory--energy drift & $4.35\times10^{-8}$ & $<1.00\times10^{-7}$ & PASS \\
\bottomrule
\end{tabular}
\end{center}
...
\caption{Three-level TDSE reference study. Time and spatial/grid refinement are performed separately. $\delta_{12}$ and $\delta_{23}$ are successive absolute differences. Both must exceed $\tau_{\rm noise}=10^{-12}+10^{-12}\max_k|O_k|$.  Retained time-refinement exponents are observed temporal orders.  Retained spatial/grid values are descriptive three-level effective contraction exponents, not demonstrated method orders.  The predeclared $p\le6$ ceiling is a conservative screening rule rather than a theoretical expected order; nonmonotone and more rapidly contracting rows are retained but not interpreted.}\label{tab:tdse-reference-refinement}\\
\toprule
...
\bottomrule
\endlastfoot
time & 20 & $\rho_{11}^{\mathrm{SN}}$ & $0.952$ & $0.952$ & $0.952$ & $1.20\times10^{-7}$ & $3.00\times10^{-8}$ & observed temporal $p=2$ \\
time & 20 & $\rho_{22}^{\mathrm{SN}}$ & $0.0481$ & $0.0481$ & $0.0481$ & $1.20\times10^{-7}$ & $3.00\times10^{-8}$ & observed temporal $p=2$ \\
...
time & 20 & $\langle R\rangle$ & $15.3$ & $15.3$ & $15.3$ & $4.91\times10^{-7}$ & $1.23\times10^{-7}$ & observed temporal $p=2$ \\
time & 20 & $\langle P\rangle$ & $19.7$ & $19.7$ & $19.7$ & $7.10\times10^{-7}$ & $1.78\times10^{-7}$ & observed temporal $p=2$ \\
time & 20 & $M^{\rm signed}_{2,R}$ & $2.41$ & $2.41$ & $2.41$ & $1.85\times10^{-6}$ & $4.64\times10^{-7}$ & observed temporal $p=2$ \\
time & 20 & $M^{\rm signed}_{2,P}$ & $2.07$ & $2.07$ & $2.07$ & $4.00\times10^{-6}$ & $1.00\times10^{-6}$ & observed temporal $p=2$ \\
time & 100 & $\rho_{11}^{\mathrm{SN}}$ & $0.143$ & $0.143$ & $0.143$ & $1.57\times10^{-6}$ & $3.92\times10^{-7}$ & observed temporal $p=2$ \\
time & 100 & $\rho_{22}^{\mathrm{SN}}$ & $0.857$ & $0.857$ & $0.857$ & $1.57\times10^{-6}$ & $3.92\times10^{-7}$ & observed temporal $p=2$ \\
...
time & 100 & $\langle R\rangle$ & $14.9$ & $14.9$ & $14.9$ & $1.81\times10^{-7}$ & $4.54\times10^{-8}$ & observed temporal $p=2$ \\
time & 100 & $\langle P\rangle$ & $99.1$ & $99.1$ & $99.1$ & $1.58\times10^{-6}$ & $3.94\times10^{-7}$ & observed temporal $p=2$ \\
time & 100 & $M^{\rm signed}_{2,R}$ & $1.01$ & $1.01$ & $1.01$ & $4.65\times10^{-8}$ & $1.16\times10^{-8}$ & observed temporal $p=2$ \\
time & 100 & $M^{\rm signed}_{2,P}$ & $0.376$ & $0.376$ & $0.376$ & $1.12\times10^{-6}$ & $2.79\times10^{-7}$ & observed temporal $p=2$ \\
...
grid & 20 & $M^{\rm signed}_{2,R}$ & $2.41$ & $2.41$ & $2.41$ & $2.10\times10^{-12}$ & $3.15\times10^{-12}$ & roundoff- or saturation-limited; order not interpreted \\
grid & 20 & $M^{\rm signed}_{2,P}$ & $2.07$ & $2.07$ & $2.07$ & $5.91\times10^{-12}$ & $3.58\times10^{-12}$ & three-level $p_{\rm eff}=0.723$ \\
...
grid & 100 & $M^{\rm signed}_{2,R}$ & $1.01$ & $1.01$ & $1.01$ & $0$ & $0$ & roundoff- or saturation-limited; order not interpreted \\
grid & 100 & $M^{\rm signed}_{2,P}$ & $0.376$ & $0.376$ & $0.376$ & $0$ & $0$ & roundoff- or saturation-limited; order not interpreted \\
\end{longtable}
\endgroup
\end{landscape}
...
\setlength{\tabcolsep}{3.5pt}
\begin{longtable}{l r l r r r r r L{0.18\textwidth}}
\caption{Three-level grid QCLE reference study. Time and spatial/grid refinement are performed separately. $\delta_{12}$ and $\delta_{23}$ are successive absolute differences. Both must exceed $\tau_{\rm noise}=10^{-12}+10^{-12}\max_k|O_k|$.  Retained time-refinement exponents are observed temporal orders.  Retained spatial/grid values are descriptive three-level effective contraction exponents, not demonstrated method orders.  The predeclared $p\le6$ ceiling is a conservative screening rule rather than a theoretical expected order; nonmonotone and more rapidly contracting rows are retained but not interpreted.}\label{tab:qcle-reference-refinement}\\
\toprule
Refinement & $P_{\rm init}$ & Observable & Level 1 & Level 2 & Level 3 & $\delta_{12}$ & $\delta_{23}$ & Interpretation \\
\midrule
\endfirsthead
...
\bottomrule
\endlastfoot
time & 20 & $\rho_{11}^{\mathrm{SN}}$ & $0.948$ & $0.948$ & $0.948$ & $1.27\times10^{-10}$ & $7.86\times10^{-12}$ & observed temporal $p=4.01$ \\
time & 20 & $\rho_{22}^{\mathrm{SN}}$ & $0.0518$ & $0.0518$ & $0.0518$ & $1.27\times10^{-10}$ & $7.86\times10^{-12}$ & observed temporal $p=4.01$ \\
...
time & 20 & $M^{\rm signed}_{2,R}$ & $2.14$ & $2.14$ & $2.14$ & $4.36\times10^{-9}$ & $2.76\times10^{-10}$ & observed temporal $p=3.98$ \\
time & 20 & $M^{\rm signed}_{2,P}$ & $1.05$ & $1.05$ & $1.05$ & $1.46\times10^{-9}$ & $1.09\times10^{-10}$ & observed temporal $p=3.75$ \\
time & 100 & $\rho_{11}^{\mathrm{SN}}$ & $0.143$ & $0.143$ & $0.143$ & $1.68\times10^{-10}$ & $9.50\times10^{-12}$ & observed temporal $p=4.14$ \\
time & 100 & $\rho_{22}^{\mathrm{SN}}$ & $0.857$ & $0.857$ & $0.857$ & $1.68\times10^{-10}$ & $9.50\times10^{-12}$ & observed temporal $p=4.14$ \\
...
time & 100 & $M^{\rm signed}_{2,R}$ & $1.01$ & $1.01$ & $1.01$ & $1.42\times10^{-11}$ & $7.67\times10^{-13}$ & roundoff- or saturation-limited; order not interpreted \\
time & 100 & $M^{\rm signed}_{2,P}$ & $0.382$ & $0.382$ & $0.382$ & $1.62\times10^{-10}$ & $7.28\times10^{-12}$ & observed temporal $p=4.48$ \\
grid & 20 & $\rho_{11}^{\mathrm{SN}}$ & $0.412$ & $1.05$ & $0.948$ & $0.638$ & $0.102$ & three-level $p_{\rm eff}=2.64$ \\
grid & 20 & $\rho_{22}^{\mathrm{SN}}$ & $0.595$ & $-0.0502$ & $0.0518$ & $0.645$ & $0.102$ & three-level $p_{\rm eff}=2.66$ \\
...
grid & 20 & energy & $0.101$ & $0.1$ & $0.1$ & $0.00129$ & $3.76\times10^{-5}$ & three-level $p_{\rm eff}=5.1$ \\
grid & 20 & $\langle R\rangle$ & $14.2$ & $15.5$ & $15.3$ & $1.37$ & $0.242$ & three-level $p_{\rm eff}=2.5$ \\
grid & 20 & $\langle P\rangle$ & $14.7$ & $20.1$ & $19.7$ & $5.33$ & $0.37$ & three-level $p_{\rm eff}=3.85$ \\
grid & 20 & $M^{\rm signed}_{2,R}$ & $10.7$ & $1.26$ & $2.14$ & $9.47$ & $0.874$ & three-level $p_{\rm eff}=3.44$ \\
grid & 20 & $M^{\rm signed}_{2,P}$ & $37$ & $0.451$ & $1.05$ & $36.6$ & $0.595$ & three-level $p_{\rm eff}=5.94$ \\
...
grid & 100 & $\langle R\rangle$ & $14.9$ & $14.9$ & $14.9$ & $0.00136$ & $1.71\times10^{-4}$ & three-level $p_{\rm eff}=2.98$ \\
...
grid & 100 & $M^{\rm signed}_{2,R}$ & $0.954$ & $1.01$ & $1.01$ & $0.0529$ & $5.98\times10^{-4}$ & rapid contraction; order not interpreted (not demonstrably asymptotic) \\
grid & 100 & $M^{\rm signed}_{2,P}$ & $0.51$ & $0.381$ & $0.382$ & $0.129$ & $0.00106$ & rapid contraction; order not interpreted (not demonstrably asymptotic) \\
\end{longtable}
\endgroup
\end{landscape}
...
\caption{Paired physical-error comparison against common references.  The reported difference is $\Delta E=E_{\mathrm{MIDPOINT}}-E_{\mathrm{PBME}}$ over the same four independent seeds; negative values favour MIDPOINT for that metric.  Intervals are paired Student-$t$ 95\% intervals with three degrees of freedom.  With only four paired seeds these intervals are descriptive sensitivity summaries, not strong inferential uncertainty statements; the individual paired signs in Appendix~\ref{app:complete-seed-evidence} are primary.  Raw and unit-mass density errors are distinguished so that shape normalization cannot hide normalization error.}\label{tab:paired-physical-errors}\\
\toprule
...
\bottomrule
\endlastfoot
QCLE & 20 & $R$--$P$ density & raw $E_1$ & $0.506$ & $[-0.297,1.31]$ & same positive sign across four paired seeds \\
QCLE & 20 & $R$--$P$ density & unit-mass shape $E_1$ & $2.61$ & $[-3.87,9.09]$ & same positive sign across four paired seeds \\
QCLE & 20 & $\rho_{11}^{\mathrm{SN}}$ & time-series RMS error & $0.378$ & $[-0.435,1.19]$ & same positive sign across four paired seeds \\
QCLE & 20 & $\rho_{22}^{\mathrm{SN}}$ & time-series RMS error & $0.378$ & $[-0.435,1.19]$ & same positive sign across four paired seeds \\
QCLE & 20 & $\langle P\rangle$ & time-series RMS error & $3.69$ & $[-5.32,12.7]$ & same positive sign across four paired seeds \\
QCLE & 20 & $\langle R\rangle$ & time-series RMS error & $1.97$ & $[-2.22,6.16]$ & same positive sign across four paired seeds \\
QCLE & 100 & $R$--$P$ density & raw $E_1$ & $4.89\times10^{20}$ & $[-5.69\times10^{20},1.55\times10^{21}]$ & same positive sign across four paired seeds \\
QCLE & 100 & $R$--$P$ density & unit-mass shape $E_1$ & $3.07$ & $[2.62,3.52]$ & same positive sign across four paired seeds \\
QCLE & 100 & $\rho_{11}^{\mathrm{SN}}$ & time-series RMS error & $51.4$ & $[-103,206]$ & same positive sign across four paired seeds \\
QCLE & 100 & $\rho_{22}^{\mathrm{SN}}$ & time-series RMS error & $51.4$ & $[-103,206]$ & same positive sign across four paired seeds \\
QCLE & 100 & $\langle P\rangle$ & time-series RMS error & $144$ & $[-296,584]$ & same positive sign across four paired seeds \\
QCLE & 100 & $\langle R\rangle$ & time-series RMS error & $108$ & $[-225,441]$ & same positive sign across four paired seeds \\
TDSE & 20 & $\rho_{11}^{\mathrm{SN}}$ & time-series RMS error & $0.38$ & $[-0.431,1.19]$ & same positive sign across four paired seeds \\
TDSE & 20 & $\rho_{22}^{\mathrm{SN}}$ & time-series RMS error & $0.38$ & $[-0.431,1.19]$ & same positive sign across four paired seeds \\
TDSE & 20 & $\langle P\rangle$ & time-series RMS error & $3.7$ & $[-5.3,12.7]$ & same positive sign across four paired seeds \\
TDSE & 20 & $\langle R\rangle$ & time-series RMS error & $1.98$ & $[-2.22,6.17]$ & same positive sign across four paired seeds \\
TDSE & 100 & $\rho_{11}^{\mathrm{SN}}$ & time-series RMS error & $51.4$ & $[-103,206]$ & same positive sign across four paired seeds \\
TDSE & 100 & $\rho_{22}^{\mathrm{SN}}$ & time-series RMS error & $51.4$ & $[-103,206]$ & same positive sign across four paired seeds \\
TDSE & 100 & $\langle P\rangle$ & time-series RMS error & $144$ & $[-296,584]$ & same positive sign across four paired seeds \\
TDSE & 100 & $\langle R\rangle$ & time-series RMS error & $108$ & $[-225,441]$ & same positive sign across four paired seeds \\
\end{longtable}
\endgroup
\end{landscape}
...
Because the coefficients \(c_i^n\) are signed, the bath quantities archived
under the implementation field names \(\texttt{R\_var}\) and
\(\texttt{P\_var}\) require an explicit definition. Let
\begin{equation}
C_n=\sum_i c_i^n,\qquad
\langle f\rangle_{\rm signed}
=
\frac{\sum_i c_i^n f(R_i^n,P_i^n)}{C_n},\qquad C_n\ne0.
\label{appeq:signed-cloud-average}
\end{equation}
The reported central second moments are
\begin{align}
M^{\rm signed}_{2,R}
&=
\left\langle R^2\right\rangle_{\rm signed}
-\left\langle R\right\rangle_{\rm signed}^{2},
\label{appeq:signed-central-R}\\
M^{\rm signed}_{2,P}
&=
\left\langle P^2\right\rangle_{\rm signed}
-\left\langle P\right\rangle_{\rm signed}^{2}.
\label{appeq:signed-central-P}
\end{align}
These expressions coincide with ordinary variances only when the normalized
measure is physically admissible and nonnegative. For a sign-indefinite or
otherwise nonphysical output they need not be nonnegative. Accordingly, the
thesis displays \(M^{\rm signed}_{2,R}\) and \(M^{\rm signed}_{2,P}\), not
``variance.'' A negative value is classified as a physical-admissibility
failure of the signed output and is never interpreted as a negative physical
variance. If \(C_n\) is zero or nonfinite, the normalized moment is undefined
and the output fails the same admissibility screen.
...
The compact printed tables are accompanied by full-precision machine-readable
CSV files in the release archive. In particular,
\texttt{support/cloud\_size\_verdict\_audit.csv} records every stage of the
Table~\ref{tab:independent-cloud-size} decision hierarchy, and
\texttt{reference\_grid\_qcle/qcle\_reference\_accuracy.csv} records the
declared tolerances, usable Richardson-style estimates, and reference roles.
Before reading any dense table below, use
\texttt{TABLE\_DATA\_CROSSWALK.csv} to locate its corresponding full-precision
CSV; the printed tables are compact views, while the CSVs are the authoritative
machine-readable records. The institutional \TeX{} package is portable only
as the complete supplied directory tree because its relative inputs, figures,
tables, bibliography, and institutional style files are preserved together.
...
\input{../final_reviewer_closure/tables/ManufacturedComplete.tex}
...
within-seed ratio, paired difference, contraction flag, numerical floor,
physical-admissibility status, and resulting hierarchy verdict. The raw
cross-seed observable spread remains visible only as a descriptive
cloud-variability diagnostic and is not used as an order or uncertainty gate.
\input{../final_reviewer_closure/tables/TimeStepRunByRun.tex}
...
\input{../final_reviewer_closure/tables/InitialLabelDistribution.tex}
\input{../final_reviewer_closure/tables/TailSensitivity.tex}
...
\input{../final_reviewer_closure/tables/DensityReferenceErrorsPerSeed.tex}
\input{../final_reviewer_closure/tables/ObservableReferenceErrorsPerSeed.tex}
...
\url{https://github.com/sahandgit/gaussian_process_guided_mapping_quantumclassical_liouville_dynamics/releases/download/thesis-final-2026-08-01-r4/frozen_numerical_evidence_payload.zip}.\\
Final release tag (identifies the final tagged document commit):
\path{thesis-final-2026-08-01-r4}.\\
Archive filename:
\path{frozen_numerical_evidence_payload.zip}.\\
Frozen source/evidence commit (retrievable at the repository above):
\texttt{ed4f6bac\allowbreak{}043e08b8\allowbreak{}554744c7\allowbreak{}cd441f9c\allowbreak{}f0e92801}.\\
Frozen numerical-evidence archive SHA-256:
\texttt{bc6dc033\allowbreak{}30c11a37\allowbreak{}9d4c4903\allowbreak{}0e88e7bd\allowbreak{}ae1aa696\allowbreak{}873c6a52\allowbreak{}e9fd9dd8\allowbreak{}e2300d0b}.\\
Embedded checksum-index SHA-256:
\texttt{db64efb1\allowbreak{}0f58bddd\allowbreak{}24eb5d2d\allowbreak{}0c9ab5c6\allowbreak{}fd990f0d\allowbreak{}cfa282a9\allowbreak{}1ff46e14\allowbreak{}87c9023b}.
The archive contains \path{final_reviewer_closure/README.md} (reproduction instructions), \path{environment.json} and \path{environment/pip_freeze.txt} (environment), \path{FINAL_RUN_MANIFEST.json} and the table/figure and per-run manifests, plus \path{PAYLOAD_SHA256SUMS.csv} for every archived file. The release also supplies \path{CLEAN_ROOM_VERIFICATION.json}, which records public download, checksum, extraction, manifest, and clean source-compilation checks. The public release asset and cited frozen source/evidence commit make this record independently retrievable and checksum-verifiable. The versioned GitHub release is not a DOI or an institutional persistent identifier.
\endgroup
