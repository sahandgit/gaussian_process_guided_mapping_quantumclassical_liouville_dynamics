...
def write_reproducibility_docs(out: Path) -> None:
    ...
    readme = f"""# Final reviewer closure evidence

This directory is generated by `{REPO / 'reviewer_final_closure.py'}`.
...
Run from `{REPO}` with `{sys.executable}`:
...
def verify(args: argparse.Namespace) -> int:
    ...
    result = {
        ...
        "checks": checks,
        "permanent_archive_status": "BLOCKED_EXTERNAL_PUBLICATION",
        "permanent_archive_note": (
            "A real DOI or permanent release identifier requires author login "
            "to a repository; no identifier is invented."
        ),
    }
...
def package(args: argparse.Namespace) -> int:
    ...
    write_json(out / "archive_manifest.json", {
        ...
        "permanent_identifier_status": "BLOCKED_EXTERNAL_PUBLICATION",
    })
    print(f"[package] archive: {archive}")
    print(f"[package] SHA-256: {archive_sha}")
    print("[package] permanent identifier: BLOCKED_EXTERNAL_PUBLICATION")
    return 0
